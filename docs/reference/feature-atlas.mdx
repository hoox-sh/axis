...
| Built-in script catalog | `src/indicators/builtins/` |
...
| First-party built-in scripts | `src/indicators/builtins/` |
...
| MCP server | `worker/src/mcp/` (JSON-RPC `/mcp` + `McpBridgeDO`) |
| MCP PWA host | `src/mcp/` (capabilities, snapshot, WS bridge) |
| MCP CLI | `packages/cli/src/commands/mcp.ts` (`axis mcp`) |
...
| First-party built-in Pine | `src/indicators/builtins/` |
| Unit tests | `tests/**` |
...
| Worker script | `worker-axis` |
| Worker URL | `https://worker.axis.hoox.sh` |
| Health service | `worker-axis` |
