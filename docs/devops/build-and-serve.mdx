...
Production AXIS assets are a **static site** produced by Vite (`bun run build` → `dist`). Serve them with any static host, Cloudflare Pages, or the repo’s `axis_pwa_server.py` SPA-aware server.
...
Behavior (`axis_pwa_server.py`):

| Rule | Detail |
| --- | --- |
| Root | Serves `dist/` |
| Cache | `/assets/*` → `immutable` long cache; else `no-cache` |
| `GET /health` (`/healthz`) | `200` `text/plain` `ok` — never SPA-fallback to `index.html` |
| `OPTIONS` | `204` with CORS headers (not 501) |
| `/favicon.ico` | Serves `assets/icon-192.png` when present |
| SPA fallback | Unknown paths → `/index.html` |
| **Never rewrite** | `/health`, `/favicon.ico`, `/assets/`, `/plugins/`, `/vendor/`, `/pyodide/`, or extensions including `.js/.css/.whl/.wasm/.zip/.py/.json…` |
...
| `axis_pwa_server.py` | Threading HTTP SPA server |
| `src/engines/catalog.ts` | Asset URL expectations |
| `LEGACY.md` | Product path vs old shell |
