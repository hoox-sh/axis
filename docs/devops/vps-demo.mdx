...
- nginx proxies SPA to static server and Pro API paths (`/run`, `/ws/`, `/health`,
...
# 2. On VPS (paths may vary — often /opt/axis)
ssh <vps> 'cd /opt/axis && git pull && bun install && bun run build && sudo systemctl restart axis-pwa'
# Pro API (sister repo) if needed:
# ssh <vps> 'cd /opt/pynescript && git pull && sudo systemctl restart pynescript-api'
```
