...
| Dev + Flask | + `make -C ../pyne run` (checkout sometimes named `pynescript`) | `:5002` |
| Static dist | `bun run build` + `python axis_pwa_server.py` | `:8081` |
| Worker local | `bun run axis dev worker` / `cd worker && bun run dev` | `:8787` |
| CF Worker prod | `axis deploy` | HTTPS workers.dev / custom |
| Operator | `axis doctor` · `axis setup` · `axis health` | — |
...
| [OKF bundle](/axis/docs/devops/okf) | Compiled repo map, refreshed on commit |
