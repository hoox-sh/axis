...
From the pyne checkout (sister repo; clone [hoox-sh/pyne](https://github.com/hoox-sh/pyne) as `../pyne` — the local directory is sometimes still named `pynescript`):
...
Preferred operator entry for install / doctor / setup / deploy — see [AXIS CLI](/axis/docs/devops/cli):
...
| Target | Intent |
| --- | --- |
| `python axis_pwa_server.py` | Serve built `dist/` on `:8081` (not Vite) |
| `make worker-dev` | Wrangler dev |
| `make worker-install` | Install worker deps |
| `bun run test:unit` | Bun unit tests under `tests/` |
...
| `vite.config.*` | Vite / Solid plugin |
| `src/engines/catalog.ts` | Default engine endpoint |
| `worker/wrangler.toml` | Local vars |
| `public/plugins/` | Example modules served at `/plugins/` |

## Worked loop

1. Terminal A: `make run` (Flask).
2. Terminal B: `bun run dev`.
...
4. **`make`** prints the Vite URL `:3000`. Compose serves the built PWA on `:8081`.
