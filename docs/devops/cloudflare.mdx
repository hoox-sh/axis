...
description: "Pages + Worker for AXIS: frozen project pynescript-axis, bindings, deploy order, EXTERNAL_BACKEND reality."
---
...
# or: axis setup d1 --remote && axis deploy worker
...
bun install
bun run build
npx wrangler pages deploy dist --project-name=pynescript-axis
```

Worker package also defines:

```json
"deploy:pages": "wrangler pages deploy ../dist --project-name=pynescript-axis"
```
