...
Every **push to `main`** (and PRs that touch desktop/frontend paths) runs
[`.github/workflows/desktop.yml`](https://github.com/hoox-sh/axis/blob/main/.github/workflows/desktop.yml):
...
Artifacts are named `axis-desktop-{linux\|macos-arm64\|macos-x64\|windows}-{sha}`.
Concurrency cancels superseded runs on the same ref so each push rebuilds cleanly.
...
| Engine | Notes |
|--------|--------|
| Local **pyne** Pro API | Clone [hoox-sh/pyne](https://github.com/hoox-sh/pyne) as `../pyne` (`make run` → `:5002`; Vite proxies `/run` in dev). Local checkouts are sometimes still named `pynescript`. |
