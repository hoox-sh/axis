...
AXIS quality gates live primarily under the repo root (Bun tests) and `worker/tests/`, orchestrated by GitHub Actions (`.github/workflows/ci.yml`, `axis-nightly.yml`). Deep test conventions are documented in [Testing reference](/axis/docs/reference/testing) and `TESTING.md`.
...
| Path | Role |
| --- | --- |
| `TESTING.md` | Authoritative testing guide |
| `scripts/check-coverage.mjs` | Scoped line gate |
| `playwright.config.ts` | webServer build+preview `:4173` |
| `e2e/` | Specs with `@smoke` / `@critical` tags |
| `worker/tests/` | auth, keys, scripts, runtime |
