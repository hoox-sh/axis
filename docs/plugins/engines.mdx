...
AXIS Worker in-process Pyodide (`PYODIDE_IN_WORKER`) remains a separate, feature-gated edge path — see [Worker runtime](/axis/docs/worker/runtime).
...
flowchart TB
  ...
  WRK -.->|PYODIDE_IN_WORKER=enabled| EDGE[In-worker Pyodide]
...
3. `micropip.install` same-origin `/vendor/pynescript-0.5.0-py3-none-any.whl` and the antlr4 4.13.2 wheel.
...
| Path | Role |
| --- | --- |
| `src/engines/catalog.ts` | `serverEngine`, `pyneWorkerEngine`, `pyodideEngine` |
| `src/indicators/runner.ts` | UI run orchestration |
| `public/vendor/*.whl` | Shipped wheels |
| `public/pyodide/` | Self-hosted Pyodide + runtime py |
| `worker/src/runtime.ts` | Edge `/api/run` |
| `worker/RUNTIME.md` | Gated in-worker Python (`PYODIDE_IN_WORKER`, default disabled) |
...
See [plugin examples](/axis/docs/reference/plugin-examples) — `example-tiny-pyne-engine.js` implements a JS DSL with `sma` / `ema` / `rsi` for demos without PYNE.
...
- [Evaluation map](/axis/docs/architecture/evaluation) — Flask vs Pyodide vs Worker vs (not) PyneTS
- [Worker runtime](/axis/docs/worker/runtime)
- [Build and serve](/axis/docs/devops/build-and-serve)
- [PYNE runtime](/pyne/docs/runtime)
- [PyneTS](/pyne/docs/pynets) — TS library; AXIS does not import it
