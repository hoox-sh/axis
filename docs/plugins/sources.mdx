...
| `geckoterminal-ohlcv` | GeckoTerminal DEX | yes | Pool candles via the Worker proxy. A **source**, not a dataset plugin. |
| `ccxt-rest` | CCXT (Gateway) | yes | Long-tail OHLCV through the datafeed gateway (`pyne` or sidecar). Credentials stay server-side. |
...
| `data-manager` | Data Manager cache | cache, then venue | Local bars-cache, then gap-fill via the venue REST source |
...
| [Data Source Manager](/axis/docs/enduser/guides/data-source-manager) | Multi-page walk-back with **`endTime` only** per page, then **validate + gap-fill**, durable IDB cache |
