...
The DO supports all 5 built-in venues (Binance, OKX, Bybit, Coinbase, Kraken) via the `venue` query param. Each venue's WS URL and subscribe message is built from `src/streams/ws-venues.ts` — the same shared helper used by browser-side StreamPlugins.
