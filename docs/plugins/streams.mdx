...
A **stream** advances the chart’s present. It opens a connection (or timer), emits `onBar` updates, and returns a **stop** function. Built-ins live in `src/streams/catalog.ts`. An optional Cloudflare Durable Object relay lives as a loadable example (`example-cf-do-stream.js`) plus Worker `SessionDO`.
...
| `mexc-ws` | MEXC WebSocket | no | JSON kline (`wss://wbs.mexc.com/ws`); 15s PING |
| `ccxt-ws` | CCXT WS (Gateway) | no | Live klines through the datafeed gateway WebSocket |
...
| Path | Role |
| --- | --- |
| `src/streams/catalog.ts` | Built-ins + register helpers |
| `src/streams/ws-venues.ts` | Shared venue WS URL + subscribe builders |
| `src/streams/binance.ts` | Shared Binance helpers (if split) |
| `src/streams/multiplex.ts` | Multi-symbol helpers |
| `worker/src/durable-objects/session.ts` | `SessionDO` multi-venue fan-out |
| `public/plugins/example-cf-do-stream.js` | Loadable DO client stream |
...
2. Client opens `wss://…/api/stream?session=…&venue=binance&symbol=BTCUSDT&interval=1m`.
3. Worker routes to `SessionDO`; DO opens one upstream WS for the venue and broadcasts raw kline JSON.
...
Built-in venue streams use `openReconnectableWs` (`src/streams/reconnect-ws.ts`):
