...
`PluginRegistry` (`src/plugins/registry.ts`) is the **single in-memory source of truth** for every installed plugin of every kind. Catalogs, the dynamic loader, active resolution, and Manager UI all read/write through this class (or thin wrappers that call it).
...
| Concern | Path |
| --- | --- |
| Class + singleton | `src/plugins/registry.ts` |
| Idempotent builtins | `src/plugins/bootstrap.ts` → `ensureBuiltins()` |
