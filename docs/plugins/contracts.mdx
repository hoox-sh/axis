...
All AXIS plugins share `PluginBase` and a discriminant `kind`. Contracts live in `src/plugins/types.ts`. Catalogs and the dynamic loader assert these shapes before registration; the UI never calls plugins that failed assertion.
