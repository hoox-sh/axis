...
A **dataset** plugin loads an alternate series through `fetchDataset`. The only built-in is `defillama-tvl` (`BUILTIN_DATASETS` in `src/onchain/catalog.ts`). GeckoTerminal pool candles are source `geckoterminal-ohlcv`, with the client in `src/onchain/geckoterminal.ts` and the Worker gecko proxy. They are not a dataset id. Derived TVL spike events come from `src/onchain/events.ts`. UI: [On-Chain panel](/axis/docs/enduser/guides/on-chain).
...
| --- | --- | --- | --- |
| `defillama-tvl` | DatasetPlugin | yes | Protocol TVL history; browser uses the Worker llama proxy by default |
| `geckoterminal-ohlcv` | source, not a dataset | yes | DEX pool OHLCV. Client `src/onchain/geckoterminal.ts`; browser uses the Worker gecko proxy |
