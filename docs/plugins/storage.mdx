...
**Storage** plugins persist the user’s Pine library and drafts. Built-ins: `local`, `cloud`, `git` — registered from `src/storage/catalog.ts`. High-level UI/editor APIs go through `src/storage/service.ts`, which always dual-writes drafts to **local** for crash recovery.
