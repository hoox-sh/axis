...
2. Settings → Script storage → Cloud (Worker), or Manager → Script Library → backend `cloud`
3. Worker URL `http://127.0.0.1:8787`, API key from **Generate demo key** (local `ALLOW_OPEN_KEYS=1`) or `/api/keys`
