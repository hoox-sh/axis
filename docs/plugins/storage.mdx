...
| Version history | `listVersions` / `readAtRevision` — GitHub/GitLab commits for the script path |
...
| `supportsScriptVersioning` | True when active storage implements git history |
| `listScriptVersions(id)` | Commit history for a script (git only) |
| `readScriptVersion(id, rev)` | Content at a commit SHA (git only) |
