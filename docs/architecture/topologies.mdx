...
---
title: "Topologies"
description: "AXIS deploy and dev topologies: Vite+Flask, static PWA, Cloudflare Pages+Worker, offline Pyodide lab, and local Bun sidecar."
---
...
```mermaid
flowchart TB
  U[Browser] --> P[CF Pages axis.hoox.sh]
  U --> W[Worker worker.axis.hoox.sh]
...
**Ids:** Pages project `axis` at `https://axis.hoox.sh`; Worker script `worker-axis` at `https://worker.axis.hoox.sh` ([ADR-008](/axis/docs/architecture/adrs)).
...
```mermaid
flowchart LR
  V[Vite AXIS app :3000] --> S[Bun sidecar :5003]
  S -->|REST OHLCV| EX[CCXT exchanges]
  S -->|WS watch| EX
  V -->|POST /run| F[Flask :5002]
  F --> PYNE[pynescript runtime]
  V -->|direct WS| BN[Venue APIs]
```
