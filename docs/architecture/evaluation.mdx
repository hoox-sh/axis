...
| Engine `pyodide` | same file | Self-hosted **Pyodide 0.26.2** (`/pyodide/v0.26.2/`), wheels `/vendor/pynescript-0.4.0-py3-none-any.whl` + antlr4, runtime `/pyodide/pynescript_runtime.py`. **Numba is not in Wasm**; `compile` / `auto` is object-mode. Cold load ~20–30s. |
...
| HOOX **pyne-worker** | `src/workers/catalog.ts` | Not a built-in engine. Paste origin as Backend URL. Distinct from the AXIS Worker (`worker-axis`). |
