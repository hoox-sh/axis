...
`SessionDO` (`worker/src/durable-objects/session.ts`) is a **per-session WebSocket relay**. `McpBridgeDO` (`worker/src/mcp/bridge.ts`) is the **MCP control plane** for a connected PWA (see [MCP server](/axis/docs/worker/mcp)). Browsers connect to the Worker at `/api/stream`; the Worker pins a Durable Object by session name and the DO opens **one** upstream Binance kline socket, broadcasting payloads to connected clients.
...
`public/plugins/example-cf-do-stream.js` maps kline fields to `Bar` and builds the Worker URL from config `endpoint`.
