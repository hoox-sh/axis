...
The Worker **data plane** for user scripts is `worker/src/scripts.ts` — REST under `/api/scripts` with Bearer auth ([auth](/axis/docs/worker/auth)). Persistence prefers **D1** (`env.DB`); without D1, per-isolate **in-memory** maps support local `wrangler dev`.
