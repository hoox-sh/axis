...
Same `requireApiKey` as `/api/scripts`. Discovery `GET /mcp` is public. App-plane tools additionally need a PWA WebSocket on `/api/mcp/bridge` (same Bearer / `?key=`). See [MCP server](/axis/docs/worker/mcp).
