...
Same `requireApiKey` as `/api/scripts`. Discovery `GET /mcp` is public. App-plane tools additionally need a PWA WebSocket on `/api/mcp/bridge`: mint a 30s one-time ticket with `GET /api/mcp/bridge?issue=ticket` (`Authorization: Bearer`), then connect `wss://…/api/mcp/bridge?ticket=…`. WS upgrade rejects the long-lived `?key=` (`NO_KEY`). `?key=` remains HTTP `extractBearer` only (curl on `/mcp` / `/api/scripts`). See [MCP server](/axis/docs/worker/mcp).
