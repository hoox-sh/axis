...
**Project name is frozen:** `name = "pynescript-axis"`.
...
| `ALLOWED_ORIGIN` | `https://pynescript.ai` | Extra CORS allowlist (comma-separated); product hosts built-in |
...
Static PWA assets are **not** served by this Worker in the documented split: **Pages** serves `dist/`, Worker serves API/WS. Project name for Pages deploy remains `pynescript-axis`.
