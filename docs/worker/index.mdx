...
**Honest runtime fact:** `POST /api/run` **proxies to `EXTERNAL_BACKEND`** (typically Flask) unless `PYODIDE_IN_WORKER=enabled` and the in-worker path succeeds. In-worker Pyodide is **planned / feature-gated** — see [Runtime](/axis/docs/worker/runtime) and `worker/RUNTIME.md`.
...
| Surface | Value |
| --- | --- |
| Wrangler / Pages project | **`pynescript-axis`** — **do not rename** lightly |
