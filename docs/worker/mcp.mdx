...
Auth: same **Bearer API key** as `/api/scripts` (`pn_…`). `GET /mcp` is public discovery. Production fail-closes when D1 is bound without `API_KEYS` KV. Rate limit: 60 RPC methods / minute / key (JSON-RPC batches charge per item, max 10; bodies above 1 MiB are rejected before parse).
...
| `/api/mcp/bridge` | WS | `?ticket=` (one-time, minted below) or `Authorization` header | PWA control plane → `McpBridgeDO`. Browser sockets cannot set Authorization; they never put the long-lived `pn_…` key in the query string. |
| `/api/mcp/bridge` | GET | Bearer | No `Upgrade` header → DO `/status`: `{ status, connected }` tab count (the app status bar polls this) |
| `/api/mcp/bridge?issue=ticket` | GET | Bearer | Mint a single-use WS ticket (`{ ticket, expiresIn }`, 30s TTL) |
