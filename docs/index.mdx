...
**AXIS** is an installable charting PWA (v**2.13.0**, `package.json` / `VERSION`) that treats **price**, **time**, and **calculation** as separable axes. Release history is `CHANGELOG.md`. Sources load history. Streams advance the present. Engines evaluate Pine Script™ via **PYNE**. Storage keeps your script library. Compose any lawful combination without a proprietary charting host.
...
5. **MCP** on the Worker (`POST /mcp`) so agents can drive APIs and a connected tab — [MCP (agents)](/axis/docs/enduser/guides/mcp)
...
On-chain dataset plugin: defillama-tvl only. Pool candles are source geckoterminal-ohlcv, not a dataset id.
```
...
`llm.txt`, `llms.txt`, and the track PDFs are generated outside this package. AXIS `package.json` has no `docs:exports` script. The generator is `node scripts/docs/generate-exports.mjs` in the PYNE repo, and the landing site copies these pages when it builds.
