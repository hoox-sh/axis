...
<img src="/images/app/chart-panes.png" alt="Price, volume, and RSI panes with drawing toolbar" />
...
| `plot.style_linebr` / `steplinebr` / `areabr` | Broken line / stepline / area — `na` ends a segment (LWC would otherwise connect through whitespace) |
...
<img src="/images/app/chart-layout-2x2.png" alt="2×2 multi-chart grid with BTCUSDT loaded in the active slot" />
...
<img src="/images/app/bar-replay.png" alt="Bar replay strip on BTCUSDT: scrub, play, and speed" />

<img src="/images/gifs/bar-replay.gif" alt="Scrubbing and playing bar replay over loaded history" />
...
<img src="/images/app/compare.png" alt="ETHUSDT percent overlay compared against BTCUSDT" />

<img src="/images/app/layers.png" alt="Layers panel: panes, volume profile overlay, scripts, drawings" />
...
| File / code | Load that script's source into the docked editor |
...
Module-bar **camera** (before Studio) captures Lightweight Charts panes to PNG:

| Option | Meaning |
| --- | --- |
| Price / Panes / Workspace | Price only, all visible panes stacked, or stacked panes plus a header band |
| Drawings | Composite the SVG drawing overlay onto the pane |
| Watermark | `AXIS · symbol · interval · time` footer |
| Scale | 1× native chart pixels, or 2× |
| Copy / Download | Clipboard `image/png`, or `axis-SYMBOL-TF-timestamp.png` |

⌘K → **Screenshot chart…** / **Copy screenshot**. Last options persist in `localStorage`.

### Drawings toolbar

See [Drawings](/axis/docs/enduser/guides/drawings). Tools include cursor, hline, vline, trend, ray, extend, rect, ellipse, arrow, fib, measure, text (plus fib/shape/annotation/trading packs). The style bar edits color/width/dash plus per-tool extras (extend, fib levels, RR, text); last-used style is stored per kind. Templates and duplicate helpers live under Layers / drawings modules.
