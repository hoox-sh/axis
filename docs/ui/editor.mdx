...
Topbar **Profiler** sets `store.profilerEnabled` (persisted). When on, the editor header shows last-run latency (`lastRunMs`); engines may also supply `profile.lines` for per-line % gutters. Details: [Debugging](/axis/docs/ui/debugging).

## See also

- [Debugging](/axis/docs/ui/debugging) — Pine Logs + Profiler
