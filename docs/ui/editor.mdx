...
Sources: TradingView *Exploring Unicode*, [Text and shapes](https://www.tradingview.com/pine-script-docs/visuals/text-and-shapes/), box-drawing U+2500, PineCoders conventions.
...
Hover + param checklist are writing-time docs — not a TradingView® debug API. Pre-eval marks the buffer; chips still read **last run**. Details: [Debugging](/axis/docs/ui/debugging).

### Diagnostics & Problems

**Pre-eval (as you type)** — `schedulePreeval` waits `PREEVAL_IDLE_MS` (1000 ms) after the last keystroke (marks clear while typing; mid-token is never flagged). Local parse/lint (`localPreevaluate`) plus `POST /lsp/diagnostics` when engine=server. Severity **error** disables Run (topbar, editor, Mod-Enter, command palette). Warnings and typos do not block.
