...
The docked editor uses **FloatableShell** (same hamburger dock menu / float / drag as other panels). Header tools:

- **Scriptlogs** — last-run `log.*` panel  
- **Profiler** — sets `store.profilerEnabled` (persisted); shows last-run latency; engines may supply `profile.lines` for per-line % gutters  

Details: [Debugging](/axis/docs/ui/debugging).

## See also

- [Debugging](/axis/docs/ui/debugging) — Scriptlogs + Profiler
