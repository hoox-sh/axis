...
| MCP | `src/mcp/dispatch.ts` reads/writes allowlisted store paths; snapshots redact secrets |
...
| watchlist | majors on **Main**, 15s refresh; named lists persist |
...
3. Key migration is read-repair ([state namespaces](/axis/docs/architecture/state-namespaces)).  
