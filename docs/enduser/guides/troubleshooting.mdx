...
5. **MCP plane** — `/mcp` auth, PWA bridge  
...
| Symptom | Checks | Fix |
| --- | --- | --- |
| Live toggle no effect | Live is off (`stream: null`) | Select `binance-ws` or `mock-poll` |
...
| Symptom | Checks | Fix |
| --- | --- | --- |
| Indicator blank / missing on chart | Oscillator on price scale; wrong pane id | Prefer `overlay=false`; re-run; see [Indicators](/axis/docs/ui/indicators) |
...
| No Debug chips / Pins | Toggles + log shape | Enable **Debug** / **Pins**; logs need `line` and/or `bar_index` — [Debugging](/axis/docs/ui/debugging) |
| Completions thin | Builtins corpus | Re-sync `pyne-builtins.json` from PYNE; optional remote LSP |
...
| Lost multi-chart slots | `chartLayout` in store | Named layouts menu; re-apply recipe |

## MCP

| Symptom | Checks | Fix |
| --- | --- | --- |
| 401 on `/mcp` | Bearer `pn_…` | `axis keys create`; client `Authorization` header |
| `SESSION_OFFLINE` | Tab open? Same key? | Settings → MCP on; Settings → General → Worker (cloud + MCP) has the key |
| `NO_MCP_BRIDGE` | Worker health `features.mcpBridge` | Bind `MCP_BRIDGE` in wrangler.toml and deploy |
| Bridge idle | Cloud API key | Settings → General → Worker (cloud + MCP) |

Guide: [MCP (agents)](/axis/docs/enduser/guides/mcp).
