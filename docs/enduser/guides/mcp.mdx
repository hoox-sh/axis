...
2. **Studio → Settings → Data** — paste Worker URL (`https://worker.axis.hoox.sh` or `http://127.0.0.1:8787`) and the `pn_…` key into cloud storage.
...
The PWA opens `wss://…/api/mcp/bridge?key=…`. The Worker partitions the session by a hash of the key — another user’s key cannot attach to your tab.

Turn the toggle **off** if you do not want agents driving this browser.

## Status-bar indicator

The status bar shows an **MCP capsule**: socket state plus how many PWA tabs
the Worker sees on your key's session (`MCP · 2 tabs`). Green means the bridge
is open; amber is connecting/retrying; dim is idle (no key). The count polls
every 20 seconds while connected. Click the capsule to open Settings → MCP.

Note: MCP clients (Claude, Cursor, …) call `POST /mcp` statelessly, so there
is no per-client presence — the capsule tracks bridge + tab attachment, which
is what `app_*` tools need.
...
| Scripts | `indicators.list` / `add` / `remove` / `update` / `run` |
...
| `Bridge: idle` / no API key | Settings → Data cloud key |
