...
- Proxy paths use the **engine endpoint**: `{endpoint}/api/onchain/llama` and `{endpoint}/api/onchain/gecko`
- Local Worker: set Backend Endpoint to `http://127.0.0.1:8787` and run `cd worker && bun run dev`
- **Not a wallet** — public metrics only; no signing

Set or change the URL under **Settings → General → Engine → Backend Endpoint**. Use **Open On-Chain panel** in that note (or topbar **Panels** / command palette) to open the panel.
