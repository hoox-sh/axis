...
<img src="/images/app/onchain-tvl.png" alt="On-Chain panel with Aave TVL attached beside the chart" />
...
<img src="/images/app/onchain-panel.png" alt="On-Chain panel: TVL / DEX / Events tabs and popular protocol presets" />

<img src="/images/gifs/onchain-attach.gif" alt="Attaching Aave TVL from the On-Chain panel onto the chart" />
...
<img src="/images/app/onchain-dex.png" alt="On-Chain DEX tab: GeckoTerminal pool OHLCV next to an attached TVL overlay" />
...
See [Data Source Manager](/axis/docs/enduser/guides/data-source-manager) for job phases and cache behavior.
...
- Worker proxy (`{endpoint}/api/onchain/llama` / `…/gecko`) is used **only** when Backend URL is a real AXIS Worker (`worker.axis.hoox.sh`, `worker-axis` / `*.workers.dev`, or wrangler `:8787`).
...
See also [CORS and origins](/axis/docs/devops/cors-and-origins) for Worker origin allowlisting.

## Dataset plugins (advanced)

On-chain providers implement the structural **dataset** plugin contract (`fetchDataset`). Built-ins register via the on-chain catalog; dynamic URL plugins may install additional datasets. End users normally only use the On-Chain panel — see [Datasets](/axis/docs/plugins/datasets) for the contract sketch.

## Related docs

- [Data Source Manager](/axis/docs/enduser/guides/data-source-manager) — OHLCV backfill (separate from on-chain scalars)
- [Sources](/axis/docs/plugins/sources) — historical bar sources
- [Worker overview](/axis/docs/worker) — `/api/onchain/…` route table
- [CORS and origins](/axis/docs/devops/cors-and-origins) — browser cross-origin constraints
