...
<img src="/images/app/data-source-manager.png" alt="Data Sources panel: symbol, venue, timeframe, and background backfill" />
...
2. Set **symbol**, **source / exchange**, **timeframe**, and **accumulate to** (UTC date). Symbol and timeframe follow the chart until you edit them.
...
5. Watch phases: **Backfilling** → **Validating** → **Filling gaps** → **Complete** (or **Partial**). Filter jobs with All / Active / Done / Issues.
6. Click **Load to chart** when ready (or rely on apply-when-complete).
7. **Browse datasets** (header icon or the button) opens the Dataset manager: coverage map, date range / max bars, then load onto the chart.
...
| `src/ui/CachedDatasetsModal.tsx` | Dataset manager modal |
| `src/ui/dsm/` | Job card, coverage map, shared formatters |
