...
**PYNE Agent** (sister project — natural language → PYNE scripts) installs from your deployed worker URL, not from AXIS `public/`:
...
| **Install** | Step-by-step setup with copyable commands (`make run`, `cd worker && bun run dev`, …) |
| **Configure** | Paste Backend URL, presets (`:5002`, `:8787`, workers.dev, axis.hoox.sh), activate Pyodide, install agent plugin |

Probes use existing health surfaces (`GET /health`, pyodide assets, `navigator.serviceWorker`) — they do not invent product APIs. Implementation: `src/workers/*`, `src/ui/WorkersManager.tsx`.
