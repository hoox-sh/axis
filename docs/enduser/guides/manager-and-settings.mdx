...
Three surfaces configure AXIS: the **Plugin Manager** (what exists and is active), the **Workers Manager** (backends / edge / Pyodide / PWA status + install helpers), and **Settings** (endpoint, engine, storage, interval defaults). None embed language semantics.
...
| Workers Manager | Topbar activity icon · ⌘K “Workers” | Probe health; set `endpoint` / engine; install PYNE Agent plugin |
...
Opens from the topbar **activity** icon (next to Plugins) or command palette **Open Workers Manager**.

| Tab | Role |
| --- | --- |
| **Overview** | Health cards for pyne Pro API, AXIS Worker (prod + local wrangler), Pyodide, PWA service worker, optional PYNE Agent / pyne-worker |
| **Detail** | Probe latency, feature flags, capabilities, Use-as-backend / preload / install actions |
| **Install** | Step-by-step setup with copyable commands (`make run`, `bun run axis dev worker`, `axis doctor`, …) |
| **Configure** | Paste Backend URL, presets (`:5002`, `:8787`, workers.dev, axis.hoox.sh), activate Pyodide, install agent plugin |

Probes use existing health surfaces (`GET /health`, pyodide assets, `navigator.serviceWorker`) — they do not invent product APIs. Implementation: `src/workers/*`, `src/ui/WorkersManager.tsx`. Operator CLI twin: [AXIS CLI](/axis/docs/devops/cli).
...
| Path | Role |
| --- | --- |
| `src/ui/PluginManager.tsx` | Manager modal |
| `src/ui/WorkersManager.tsx` | Workers Manager modal |
