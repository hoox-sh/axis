...
| **Settings** | Topbar **Studio** → Settings rail · ⌘K “Open Settings” | Appearance, chart, live, MCP bridge, session keys, editor intel, theme |
...
| **MCP** | **Connect this tab to MCP** (default on). Requires a Worker API key under Settings → General → Worker (cloud + MCP). Status line shows `open` / `idle` / `error`. See [MCP (agents)](/axis/docs/enduser/guides/mcp). |
...
| Mode | Behavior |
| --- | --- |
| `interpret` | Full AST interpreter |
| `compile` | Numba/numpy compiled path (first run JITs; Flask is not prewarmed) |
| `auto` | Default on Server / Pyodide: try compile, fall back to interpret on failure |
