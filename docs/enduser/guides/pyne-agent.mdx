...
3. Set **endpoint** to the agent Worker origin (no trailing slash), e.g. `https://pyne-agent-worker.cryptolinx.workers.dev`. Same-origin install still talks to this API host. Workers Manager → PYNE Agent also has an editable **Agent endpoint** (URL + API key).
4. Set **API key** if required.
5. Prefer **v6** / **strategy** or **indicator** when you know the target style.
6. In **Workers Manager → PYNE Agent**, pick a **Persona** (`auto` detects per message): `pine` writes scripts, `axis` answers anything about the app and can change it for you (theme, layout, runs), `trader` does market analysis and risk coaching.
