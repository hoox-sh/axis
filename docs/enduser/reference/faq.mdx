...
### Can an agent control AXIS?

Yes. The Worker serves MCP at `POST /mcp` (Bearer `pn_…`). Worker tools (`axis_run`, scripts, on-chain) do not need a tab. Chart/editor/panels need **Settings → MCP** connected. Guide: [MCP (agents)](/axis/docs/enduser/guides/mcp).

### Where do API keys live?

In **this browser’s** localStorage state / `pluginsConfig`. They are sent as HTTP headers only to endpoints you configure (cloud storage, MCP bridge, etc.). They are not part of the Pine script body. Snapshots returned to MCP clients redact secret-like fields.
...
Full workflow: [Debugging](/axis/docs/ui/debugging).
...
UI detail: [UI shell](/axis/docs/ui/ui-shell), [Charting](/axis/docs/ui/charting).
...
Yes — Results → **Optimise**, or command palette **Optimise strategy**. Strategies only. pyne `POST /optimize` runs TPE / random / grid with holdout by default; Pyodide falls back to a local random loop. See [Hyperparameter Optimisation](/axis/docs/enduser/guides/hyperparameter-optimisation).

### How do I download years of history?

Topbar **Load** is one venue page. Open the **Data** panel (Data Source Manager) to backfill in the background to a past date, validate completeness, and fill gaps — see [Data Source Manager](/axis/docs/enduser/guides/data-source-manager).
