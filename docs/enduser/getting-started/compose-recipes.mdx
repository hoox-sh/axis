...
<img src="/images/studio/wire.png" alt="Studio Wire: Live Crypto recipe with Binance REST source selected" />
...
| Kind | Built-ins |
| --- | --- |
| Source | `binance-rest`, `okx-rest`, `bybit-rest`, `coinbase-rest`, `kraken-rest`, `mexc-rest`, `geckoterminal-ohlcv`, `ccxt-rest`, `mock-walk`, `csv-upload`, `data-manager`, plus URL plugins |
| Stream | `binance-ws`, `okx-ws`, `bybit-ws`, `coinbase-ws`, `kraken-ws`, `mexc-ws`, `ccxt-ws`, `mock-poll`. There is no `none` stream. Live off is `stream: null` / `enabled: false`. |
| Engine | `server`, `pyne-worker`, `pyodide` |
...
| Multi-venue desk | binance-rest + URL sources | binance-ws, or Live off | server | local | yes | Flask |
| CSV desk | csv-upload | Live off, or mock-poll | server or pyodide | local | no* | optional |
...
| Stream | Match the venue when it has a built-in stream. Otherwise leave Live off (`stream: null`). |
...
| --- | --- |
| Source | `csv-upload` |
| Stream | Live off (typical) or `mock-poll` for artificial advance |
...
1. `wrangler dev` / deploy Worker script **`worker-axis`** (`https://worker.axis.hoox.sh`).
...
**Invariants:** Pages project is `axis` (`https://axis.hoox.sh`); Worker script is `worker-axis` (`https://worker.axis.hoox.sh`); health JSON `service` is `worker-axis`.
...
| Concern | Path |
| --- | --- |
| Active selection | `src/store` `activePlugins` |
