...
<img src="/images/app/load-ready.png" alt="AXIS after Load: BTCUSDT candles, volume, and RSI pane" />

<img src="/images/gifs/load-and-run.gif" alt="Load history then Run an RSI script" />
...
<img src="/images/app/results-strategy.png" alt="Results Strategy tab after an SMA Cross run: stats, equity, closed trades" />
...
Toggle **Live**. A stream plugin pushes bars; AXIS may re-run silently when `needsRerun` is set. Live off (`stream: null`, not a `none` plugin) freezes time. Mock poll works offline after engine warm-up.
...
| **Screenshot** | Module bar camera (before Studio) | Capture panes to PNG — drawings, watermark, 1×/2×, copy or download |
...
| **Built-in studies** | Scripts → **Built-in**, Add ▾, or ⌘K | Original AXIS RSI / MACD / SMA / … |
...
| Live no updates | Turn Live on and pick a stream (`binance-ws` or `mock-poll`); check the network and the logs drawer |
