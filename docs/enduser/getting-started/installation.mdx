...
AXIS ships as the **`axis`** package (Solid + Vite), version **2.3.1**. Primary path is the product UI under `src/`. Legacy static shell (`main.js`, root `style.css`) is not the product UI—prefer `bun run dev` or built `dist/`.
