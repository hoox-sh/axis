...
AXIS ships as the **`axis`** package (Solid + Vite), version **2.13.0**. Primary path is the product UI under `src/`. Legacy static shell (`main.js`, root `style.css`) is not the product UI—prefer `bun run dev` or built `dist/`.
...
In AXIS: open **Workers Manager** (topbar activity icon) or Settings → set **Backend URL** to `http://127.0.0.1:8787`. Production Worker is `worker-axis` at `https://worker.axis.hoox.sh` — see [topologies](/axis/docs/architecture/topologies).
