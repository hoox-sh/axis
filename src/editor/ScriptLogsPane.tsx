...
/**
 * Editor-local Script Logs pane — Pine `log.*` output from the last run.
 *
 * Renders directly above the tabbed-editor status bar (toggled by the Logs
 * button beside Symbols, or remotely via the `axis-editor-show-logs` window
 * event). Reads `store.lastRun` — the same source the former shell
 * `ScriptLogsPanel` used; rows keep jump-to-bar / flash-line behavior.
 *
 * @module editor/ScriptLogsPane
 */
...
import { flashDebugPinLine } from './inline-debug';
import { parseSourceLine } from '../results/inline-debug';
import { Icons } from '../ui/icons';
import { ScriptRunSelect } from '../ui/ScriptRunSelect';
...
function barIndexOf(entry: PyneLogEntry): number | null {
  const bi = entry.barIndex;
  if (typeof bi === 'number' && Number.isFinite(bi)) return bi;
  return null;
}
...
/** Script `log.*` output from the last run, embedded in the editor. */
export const ScriptLogsPane: Component<{ open: boolean; onClose: () => void }> = (props) => {
  ...
  createEffect(() => {
    // Auto-scroll while open and the log list grows / filter changes.
    void filtered().length;
    void allEntries().length;
    if (props.open && listRef) {
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          class="sc-btn sc-btn-ghost px-1.5 py-0.5 text-[0.78em] inline-flex items-center gap-1"
          title="Hide script logs"
          aria-label="Hide script logs"
          data-testid="axis-editor-scriptlogs-close"
              ...
              <For each={filtered()}>
                {(entry, i) => {
                  ...
                  const onJump = () => {
                    ...
                    const entryLine = (entry as { line?: unknown }).line;
                    const line =
                      parseSourceLine(entry.message) ??
                      (typeof entryLine === 'number' ? entryLine : null);
