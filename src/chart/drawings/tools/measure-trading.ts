...
registerToolHandler({
  ...
  create(points, color) {
    ...
    return { id: '', kind: 'dateRange', p1: points[0]!, p2: points[1]!, color, fillOpacity: 0.12 };
  },
  paint(d, ctx) {
    const t = asTwo(d);
    if (!t) return;
...
registerToolHandler({
  ...
  create(points, color) {
    if (points.length < 2) return null;
    return { id: '', kind: 'priceRange', p1: points[0]!, p2: points[1]!, color, fillOpacity: 0.12 };
  },
  paint(d, ctx) {
    const t = asTwo(d);
    if (!t) return;
...
function paintPosition(d: Drawing, ctx: Parameters<NonNullable<import('./registry').ToolHandler['paint']>>[1], dir: 'long' | 'short') {
  const t = asTwo(d);
  if (!t) return;
  ...
  ctx.label(x1 + 4, profitTop + 12, `${dir.toUpperCase()} ${pnl >= 0 ? '+' : ''}${pnl.toFixed(2)} (${pct.toFixed(2)}%)`, ctx.stroke, 11);
  if (ctx.selected) {
...
function hitPosition(d: Drawing, ctx: Parameters<NonNullable<import('./registry').ToolHandler['hit']>>[1]): boolean {
  const t = asTwo(d);
  if (!t) return false;
  const a = ctx.toXY(t.p1);
  const b = ctx.toXY(t.p2);
  ...
  return nearRectEdge(ctx.x, ctx.y, x1, top, x2, bot, ctx.tol) ||
