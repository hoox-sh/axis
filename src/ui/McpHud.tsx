 ...
 *
 * Click opens Settings when the bridge is up. While it is down, the chip
 * is a Connect MCP action (Settings if the Worker key is missing).
 *
 * @module ui/McpHud
 */
...
export const McpHud: Component = () => {
  ...
  const offerConnect = () => mcpNeedsConnect(s().status);
  ...
  return (
    <button
      ...
      class={
        offerConnect()
          ? 'axis-mcp-cta-chip'
          : 'inline-flex items-center gap-1.5 text-[11px] font-mono tabular-nums text-text-dim hover:text-accent flex-shrink-0'
      }
      title={
        offerConnect()
          ? 'Connect this tab to MCP. Opens Settings if no Worker key is saved.'
          : title(s(), now())
      }
      data-testid="axis-mcp-hud"
      data-connect={offerConnect() ? '1' : '0'}
      onClick={() => {
        if (offerConnect()) requestMcpConnect();
        else openSettings();
      }}
    >
      <span
      ...
      <span>{offerConnect() ? 'Connect MCP' : label(s())}</span>
    </button>
  );
};
