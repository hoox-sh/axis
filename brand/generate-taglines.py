...
"""
AXIS brand taglines — curated social/hero banners.

Headlines from https://hoox.sh/axis (hero + footer):
  1. OWN THE AXES. SWAP THE ENGINE.
  2. PRICE. TIME. ENGINE.
  3. AXIS STAYS OPEN.
...
# Landing-page headlines (hero H1, eyebrow PRICE·TIME·ENGINE, footer SEC09)
TAGLINES = [
...
KEEP_RE = re.compile(
    r"^tagline-(own-the-axes|price-time-engine|axis-stays-open)-"
...
def monogram(cx: float, cy: float, size: float, fill: str, stroke: str) -> str:
    """Cartesian axes mark + monogram — matches AXIS thesis (price × time)."""
    r = size * 0.48
    arm = size * 0.38
    tick = size * 0.06
    fs = size * 0.55
    ...
    <g stroke="{stroke}" stroke-opacity="0.55" stroke-width="1.4" fill="none" stroke-linecap="square">
      <path d="M {-arm} 0 H {arm}"/>
      <path d="M 0 {-arm} V {arm}"/>
      <path d="M {arm-tick} {-tick} L {arm} 0 L {arm-tick} {tick}"/>
      <path d="M {-tick} {-arm+tick} L 0 {-arm} L {tick} {-arm+tick}"/>
    </g>
...
def defs(t: dict) -> str:
    ...
    <filter id="monoNoise" x="0" y="0" width="100%" height="100%" filterUnits="objectBoundingBox" color-interpolation-filters="sRGB">
      <feTurbulence type="fractalNoise" baseFrequency="1.25" numOctaves="2" seed="23" stitchTiles="stitch" result="n"/>
...
def split_tagline(text: str, max_chars: int = 40) -> list[str]:
    u = text.upper()
    if "OWN THE AXES" in u and "SWAP THE ENGINE" in u:
        return ["OWN THE AXES.", "SWAP THE ENGINE."]
    if "PRICE" in u and "TIME" in u and "ENGINE" in u:
        return ["PRICE. TIME. ENGINE."]
    if "AXIS STAYS OPEN" in u:
        return ["AXIS STAYS OPEN."]
    if ". " in text:
        a, b = text.split(". ", 1)
        return [a + ".", b]
...
def main() -> int:
    ...
    print("  Taglines: OWN THE AXES. SWAP THE ENGINE. | PRICE. TIME. ENGINE. | AXIS STAYS OPEN.")
