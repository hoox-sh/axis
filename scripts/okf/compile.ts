...
function affinity(dir: string, file: ParsedCode): number {
  ...
  let score = 0;
  if (base === 'index' || base === 'main' || base === 'mod' || base === 'lib') score += 1000;
...
function linkList(dirs: string[], label: (dir: string) => string): string {
  return dirs.map((dir) => `* [${label(dir)}](/${moduleId(dir)}.md)`).join('\n');
}
  ...
  for (const dir of dirList) {
    ...
    if (concepts.length > 0) {
      ...
      for (const concept of concepts) {
        lines.push(`* [${concept.title}](${posix.basename(concept.path)}) - ${concept.description}`);
      }
      lines.push('');
    }
    if (children.length > 0) {
      ...
      for (const child of children) {
        ...
        lines.push(`* [${posix.basename(child)}](${posix.basename(child)}/) - ${blurb}`);
      }
      lines.push('');
    }
...
function renderRoot(dirs: Set<string>, all: Indexed[]): string {
  ...
  const blurbs: Record<string, string> = {
    playbooks: 'How to read this bundle and where AXIS keeps its map.',
  ...
  const lines = ['---', 'okf_version: "0.2"', '---', '', '# AXIS knowledge bundle', ''];
  for (const name of top) {
    lines.push(`# ${name[0]?.toUpperCase()}${name.slice(1)}`, '');
    if (dirs.has(name)) {
      lines.push(`* [${name}](${name}/) - ${blurbs[name] ?? 'Concepts in this section.'}`);
    }
    const rooted = all.filter((item) => item.path === `${name}.md` || posix.dirname(item.path) === name);
    if (!dirs.has(name)) {
      for (const item of rooted) {
        lines.push(`* [${item.title}](${posix.basename(item.path)}) - ${item.description}`);
      }
    }
    lines.push('');
  }
